...
source.txt의 두 문서를 각각 하나의 record로 추출한다. 문서 ID를 record_id로 유지하고 title은 서비스명, change는 변경 내용, effective_date는 적용일, owner는 담당을 원문대로 기록한다. source_file='source.txt'를 포함한다. 외부 검색과 추가 추론 없이 status=success, coverage 2/2 complete=true로 반환한다.
