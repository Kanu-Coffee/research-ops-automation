...
source.json의 metrics마다 record_id=id, title, baseline, current, delta=current-baseline, change_percent=100*delta/baseline, unit, source_file='source.json'를 반환한다. 수치는 JSON number로 쓴다. baseline이 0이면 비율을 추정하지 않고 null과 경고를 사용한다. 합성 데이터의 증가와 감소를 모두 보존한다. status=success, coverage 2/2 complete=true다.
