...
source.json의 3개 target만 검토한다. checked target의 record 1건에 source_file='source.json'을 추가하고 optional_note=null을 그대로 보존한다. unavailable/blocked는 정답을 만들어 채우지 않는다. 실제 네트워크 재시도 없이 status=partial, coverage complete=false/expected_target_count=3/completed_target_count=1, 두 issues와 warnings를 명시한다. 확인된 record를 삭제하거나 no_updates로 바꾸지 않는다.
